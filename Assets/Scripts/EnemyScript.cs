...
public class EnemyScript : MonoBehaviour
{
    ...
    private Vector3 spawn, startPoint, endPoint;
    ...
    void Update()
    {
        ...
        else
        {
            ...
            if (playerDist <= attackDistance && currentGunCooldown >= gunCooldown)
                Attack(0);
            else if (playerDist <= grappleDistance && currentGrappleCooldown >= grappleCooldown)
                Attack(1);
            else
                if (playerDist <= interceptDistance)
                    Intercept();
                else
                    Patrol();
    ...
    Vector3 GetAttackVector(float projectileSpeed)
    {
        Vector3 relvel = player.GetComponent<Rigidbody>().velocity - selfVel.velocity;
        ...
        float awayVectorMagnitude = Mathf.Sqrt(projectileSpeed * projectileSpeed - lateralVector.sqrMagnitude);
        Vector3 awayVector = awayVectorMagnitude * (player.transform.position - transform.position).normalized;
        return awayVector + lateralVector;
    }
    void Intercept()
    {
        hasGoal = false;
    ...
    void Attack(int mode)
    {
        ...
        if (mode == 1)
        {
            transform.LookAt(transform.position + GetAttackVector(grappleSpeed));
        ...
        // If grapples are not present, defaults to bullets
        else
        {
            transform.LookAt(transform.position + GetAttackVector(bulletSpeed));
